    ...
    return (
        <section id="about" className="relative overflow-hidden bg-black py-20 sm:py-24">
            <div className="mx-auto flex max-w-7xl flex-col items-start gap-12 px-5 sm:px-8 md:flex-row md:items-start md:px-16 lg:gap-16 lg:px-20">
                <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative flex flex-1 w-full justify-center md:justify-end">
                    <div className="relative w-full max-w-[400px] aspect-[3/4] md:max-w-[450px] lg:max-w-[500px] rounded-[3rem] bg-zinc-900 shadow-2xl shrink-0">
                        <img src="/Img/hector2.png" alt="Hector Garcia - About" className="h-full w-full rounded-[3rem] object-cover grayscale transition-all duration-500 hover:grayscale-0" />
                        <div className="absolute -bottom-4 -right-4 z-10 flex h-28 w-28 items-center justify-center rounded-full bg-primary ring-2 ring-primary/70 shadow-[0_14px_35px_rgba(0,230,118,0.35)] md:-bottom-6 md:-right-6 md:h-36 md:w-36 lg:h-40 lg:w-40">
                            <div className="text-center drop-shadow-md">
                                <span className="mb-1 block text-3xl font-bold text-black dark:text-white md:text-4xl text-center">1+</span>
                                <span className="block text-[10px] font-bold uppercase tracking-wider text-black dark:text-white md:text-[12px] text-center">{t("yearsExp")}</span>
                            </div>
                        </div>
                    </div>
                </motion.div>

                <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="flex-1 w-full text-center md:text-left flex flex-col pt-8 sm:pt-0">
